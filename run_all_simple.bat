...
if errorlevel 1 (
    echo ERROR Phase 1 でエラーが発生しました。ログを確認: %LOG_FILE%
    type %LOG_FILE%
    pause
    exit /b 1
)
...
if errorlevel 1 (
    echo ERROR Phase 3 でエラーが発生しました。ログを確認: %LOG_FILE%
    type %LOG_FILE%
    pause
    exit /b 1
)
...
if errorlevel 1 (
    echo ERROR Phase 4-1 でエラーが発生しました。ログを確認: %LOG_FILE%
    type %LOG_FILE%
    pause
    exit /b 1
)
...
if errorlevel 1 (
    echo ERROR Phase 4-2 でエラーが発生しました。ログを確認: %LOG_FILE%
    type %LOG_FILE%
    pause
    exit /b 1
)
...
if errorlevel 1 (
    echo ERROR Phase 5 でエラーが発生しました。ログを確認: %LOG_FILE%
    type %LOG_FILE%
    pause
    exit /b 1
)
...
if errorlevel 1 (
    echo ERROR 配信用テキスト生成でエラーが発生しました。ログを確認: %LOG_FILE%
    type %LOG_FILE%
    pause
    exit /b 1
)
...
echo 配信用: predictions\%KEIBAJO_NAME%_%DATE_SHORT%_配信用.txt
echo ログ: %LOG_FILE%
echo =============================================================================
echo.
echo 配信用テキストを表示します
