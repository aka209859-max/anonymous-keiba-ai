...
if "%~1"=="" (
    ...
    echo Example: run_all_optimized.bat 55 2026-02-07
    exit /b 1
)
...
set LOG_FILE=!LOG_DIR!\execution_optimized_!DATE_SHORT!_%TIME:~0,2%%TIME:~3,2%.log

echo ============================================================
echo 地方競馬AI予想システム Phase 7-8-5統合版（新モデル）
echo ============================================================
echo 実行開始: %DATE% %TIME%
...
REM ============================================================
REM Step 2/3: Phase 7-8 新モデルで予測
REM ============================================================
echo [Step 2/3] Phase 7-8: Optimized Predictions (Binary/Ranking/Regression)
echo ------------------------------------------------------------
echo.

REM Phase 7-8: Binary予測
set "OUTPUT_P7_BINARY=data\predictions\phase7_binary\!KEIBAJO_NAME!_!DATE_SHORT!_phase7_binary.csv"
echo [Phase 7-8 Binary] 予測実行中...
echo   入力: !FEATURES_CSV!
echo   出力: !OUTPUT_P7_BINARY!
python scripts\phase7_binary\predict_optimized_binary.py "!FEATURES_CSV!" "data\models\tuned" "!OUTPUT_P7_BINARY!"
if errorlevel 1 (
    echo [ERROR] Phase 7-8 Binary 失敗
    exit /b 1
)
echo [OK] Phase 7-8 Binary 完了
echo.
...
echo   入力: !FEATURES_CSV!
echo   出力: !OUTPUT_P8_REGRESSION!
python scripts\phase8_regression\predict_optimized_regression.py "!FEATURES_CSV!" "data\models\tuned" "!OUTPUT_P8_REGRESSION!"
if errorlevel 1 (
    echo [ERROR] Phase 8 Regression 失敗
    exit /b 1
)
echo [OK] Phase 8 Regression 完了
echo.

REM ============================================================
REM Step 3/3: Phase 5 アンサンブル統合（新モデル）
REM ============================================================
echo [Step 3/3] Phase 5: Ensemble Integration (Optimized)
echo ------------------------------------------------------------
echo.

set "OUTPUT_ENSEMBLE=data\predictions\phase5\!KEIBAJO_NAME!_!DATE_SHORT!_ensemble_optimized.csv"
echo [Phase 5 Ensemble] 統合実行中...
echo   Binary入力: !OUTPUT_P7_BINARY!
echo   Ranking入力: !OUTPUT_P8_RANKING!
echo   Regression入力: !OUTPUT_P8_REGRESSION!
echo   出力: !OUTPUT_ENSEMBLE!
...
if errorlevel 1 (
    echo [ERROR] Phase 5 Ensemble 失敗
    exit /b 1
)
echo [OK] Phase 5 Ensemble 完了
echo.

if not exist "!OUTPUT_ENSEMBLE!" (
    echo [ERROR] Phase 5 アンサンブル統合結果が見つかりません
    exit /b 1
)
...
echo ============================================================
echo 全フェーズ完了（新モデル: Phase 7-8-5）
echo ============================================================
echo 実行終了: %DATE% %TIME%
echo.
echo 出力ファイル:
echo   - Phase 7-8 Binary: !OUTPUT_P7_BINARY!
