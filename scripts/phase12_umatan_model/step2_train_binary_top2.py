...
def train_binary_model(input_dir: str, output_dir: str):
    ...
    """
    safe_print("\n" + "="*80)
    safe_print("Phase 12: Step 2 - 2着以内二値分類モデル学習【競馬場別】")
    safe_print("="*80)
    ...
    for csv_file in csv_files:
        keibajo_name = csv_file.stem  # ファイル名から拡張子を除いたもの（例: funabashi）
        safe_print(f"\n{'='*80}")
        safe_print(f"🏇 競馬場: {keibajo_name}")
        safe_print(f"{'='*80}")
        
        df_all = pd.read_csv(csv_file, encoding='shift-jis')
        safe_print(f"✅ データ読み込み完了: {len(df_all):,}行 × {len(df_all.columns)}列")
        ...
        # モデル保存（競馬場別）
        model_path = output_path / f"phase12_{keibajo_name}_binary_top2_model.txt"
        ...
        metadata_path = output_path / f"phase12_{keibajo_name}_binary_top2_metadata.json"
    ...
    # 全競馬場の結果サマリー
    safe_print("\n" + "="*80)
    safe_print("✅ Phase 12 Step 2 完了【全競馬場】")
    safe_print("="*80)
    for result in all_results:
        safe_print(f"\n{result['keibajo']:10s}: {result['n_samples']:6,}頭, AUC={result['metrics']['roc_auc']:.4f}, Acc={result['metrics']['accuracy']:.4f}")
