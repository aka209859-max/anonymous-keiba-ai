...
def train_regression_model(input_dir, output_dir):
    ...
    safe_print(f"\n✅ データ結合完了: {len(df_combined):,}行 × {len(df_combined.columns)}列")
    ...
    safe_print(f"  - keibajo_code を特徴量として含む: {'keibajo_code' in df_combined.columns}")
    
    # 特徴量の準備（keibajo_codeを特徴量に追加）
    if 'keibajo_code' in df_combined.columns:
        X = df_combined[feature_cols + ['keibajo_code']].copy()
    else:
    ...
    model_file = output_path / "phase12_regression_model.txt"
    ...
    metadata_file = output_path / "phase12_regression_metadata.json"
    with open(metadata_file, 'w', encoding='utf-8') as f:
        json.dump(metadata, f, indent=2, ensure_ascii=False)
    
    safe_print(f"💾 メタデータ保存: {metadata_file}")
    safe_print(f"\n{'='*80}")
    safe_print("✅ Phase 12 Step 4 完了")
    safe_print(f"{'='*80}")
