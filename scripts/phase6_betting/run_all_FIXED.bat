...
if "%~1"=="" (
    ...
    echo Example: run_all.bat 43 2026-02-10
    exit /b 1
)
...
if "%KEIBAJO_CODE%"=="30" set MODEL_PREFIX=monbetsu

if "%KEIBAJO_CODE%"=="35" set KEIBAJO_NAME=盛岡
if "%KEIBAJO_CODE%"=="35" set MODEL_PREFIX=morioka

if "%KEIBAJO_CODE%"=="36" set KEIBAJO_NAME=水沢
if "%KEIBAJO_CODE%"=="36" set MODEL_PREFIX=mizusawa

if "%KEIBAJO_CODE%"=="42" set KEIBAJO_NAME=浦和
if "%KEIBAJO_CODE%"=="42" set MODEL_PREFIX=urawa

if "%KEIBAJO_CODE%"=="43" set KEIBAJO_NAME=船橋
if "%KEIBAJO_CODE%"=="43" set MODEL_PREFIX=funabashi

if "%KEIBAJO_CODE%"=="44" set KEIBAJO_NAME=大井
if "%KEIBAJO_CODE%"=="44" set MODEL_PREFIX=ooi

if "%KEIBAJO_CODE%"=="45" set KEIBAJO_NAME=川崎
if "%KEIBAJO_CODE%"=="45" set MODEL_PREFIX=kawasaki

if "%KEIBAJO_CODE%"=="46" set KEIBAJO_NAME=金沢
if "%KEIBAJO_CODE%"=="46" set MODEL_PREFIX=kanazawa

if "%KEIBAJO_CODE%"=="47" set KEIBAJO_NAME=笠松
if "%KEIBAJO_CODE%"=="47" set MODEL_PREFIX=kasamatsu

if "%KEIBAJO_CODE%"=="48" set KEIBAJO_NAME=名古屋
if "%KEIBAJO_CODE%"=="48" set MODEL_PREFIX=nagoya

if "%KEIBAJO_CODE%"=="50" set KEIBAJO_NAME=園田
if "%KEIBAJO_CODE%"=="50" set MODEL_PREFIX=sonoda

if "%KEIBAJO_CODE%"=="51" set KEIBAJO_NAME=姫路
if "%KEIBAJO_CODE%"=="51" set MODEL_PREFIX=himeji

if "%KEIBAJO_CODE%"=="54" set KEIBAJO_NAME=高知
if "%KEIBAJO_CODE%"=="54" set MODEL_PREFIX=kochi

if "%KEIBAJO_CODE%"=="55" set KEIBAJO_NAME=佐賀
if "%KEIBAJO_CODE%"=="55" set MODEL_PREFIX=saga
...
echo Input: %OUTPUT_P3%, %OUTPUT_P4_RANK%, %OUTPUT_P4_REG%
...
REM Phase 5 完了後、ファイル存在確認
if not exist "%OUTPUT_ENSEMBLE%" (
    echo [ERROR] Phase 5 ensemble file not created: %OUTPUT_ENSEMBLE%
    exit /b 1
)
echo [OK] Phase 5 完了
echo [OK] Ensemble file created: %OUTPUT_ENSEMBLE%
echo.

set OUTPUT_TEXT=predictions\%KEIBAJO_NAME%_%DATE_SHORT%_配信用.txt

echo [Phase 6] 配信用テキスト生成中...
echo Input: %OUTPUT_ENSEMBLE%
...
echo 配信用テキスト: %OUTPUT_TEXT%
echo.
echo 次のステップ: DAILY_OPERATION.bat を実行してください
echo   DAILY_OPERATION.bat %KEIBAJO_CODE% %TARGET_DATE%
echo ============================================================
