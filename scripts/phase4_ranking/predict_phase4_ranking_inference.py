...
# -*- coding: utf-8 -*-
"""
Phase 4-1 ランキング学習モデルで予測を実行（予測専用版）

target列がない予測用データに対応
race_idを使用してレース内の相対順位を予測
"""
...
def predict_ranking(test_csv, model_path, output_path):
    ...
    """
    print(f"\n{'='*80}")
    print(f"Phase 4-1 ランキング予測（予測専用モード）")
    print(f"{'='*80}")
    ...
    if 'race_id' not in df.columns:
        raise ValueError("race_id列が見つかりません。ランキング予測にはrace_idが必要です。")
    
    print(f"  - ユニークレース数: {df['race_id'].nunique()}件")
    ...
    # race_idごとにランキングスコアで降順ソートして順位付け
    ...
    for race_id in sorted(result_df['race_id'].unique())[:5]:
        ...
        top_horse = race_data.nlargest(1, 'ranking_score').iloc[0]
        race_num = int(str(race_id)[-2:])
        print(f"  - 第{race_num}R: {int(top_horse['umaban'])}番 (スコア: {top_horse['ranking_score']:.4f})")
    ...
    results = {
        'Data Count': len(df),
        'Race Count': df['race_id'].nunique(),
        'Average Ranking Score': ranking_scores.mean(),
        'Max Ranking Score': ranking_scores.max(),
        'Min Ranking Score': ranking_scores.min()
    }
    
    print(f"\n[6/6] 予測結果サマリー")
    print(f"  - データ件数: {results['Data Count']:,}件")
    print(f"  - レース数: {results['Race Count']}件")
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("使用法: python predict_phase4_ranking_inference.py <test_csv> <model_path> <output_csv>")
        print("\n例:")
        print("  python predict_phase4_ranking_inference.py \\")
        print("         data/features/2026/02/川崎_20260205_features.csv \\")
        print("         models/ranking/kawasaki_2020-2025_v3_with_race_id_ranking_model.txt \\")
