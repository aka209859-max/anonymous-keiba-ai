...
Phase 7-Ranking: ランキングモデル用Boruta特徴量選択

LambdaRankモデルに最適な特徴量を選択します。

目的変数:
    - rank_target（着順の逆順: 1着=最大値）
    - またはlabel形式（相対的な順位スコア）

評価指標:
    - NDCG@5（上位5頭の順位精度）
    - またはKendall's Tau（順位相関）

使用法:
    python run_boruta_ranking.py <クリーニング済みCSV> [オプション]
...
出力:
    ...
    - data/features/selected/{venue}_ranking_importance.png
    - data/features/selected/{venue}_ranking_boruta_report.json
"""
...
class GreedyBorutaRanking:
    """
    ランキングモデル専用Greedy Borutaアルゴリズム実装
    
    二値分類と異なり、LambdaRankの目的関数に対応
    """
    ...
    def _train_ranking_model(self, X, y, groups):
        """
        ランキングモデルを学習（LambdaRank）
        
        Args:
            X: 特徴量（シャドウ含む）
            y: 目的変数（着順の逆順スコア: 1着=最大）
            groups: レースID（グループ単位でランキング学習）
        """
        # LambdaRankパラメータ
        params = {
            'objective': 'lambdarank',  # ← ランキング学習
            'metric': 'ndcg',
            'ndcg_eval_at': [5],  # 上位5頭の精度を評価
            'num_leaves': 31,
            'learning_rate': 0.05,
        ...
        for train_idx, val_idx in gkf.split(X, y, groups):
            ...
            train_data = lgb.Dataset(
                X_train, 
                label=y_train,
                group=train_group_sizes  # ← ランキング学習に必須
            )
        ...
        return dict(zip(X.columns, avg_importance))
    ...
    def fit(self, X, y, groups):
        ...
        Args:
            X: 特徴量DataFrame
            y: 目的変数（rank_target: 1着=最大値）
            groups: レースID（race_id）
        
        Returns:
            list: 選択された特徴量リスト
        """
        print("\n" + "=" * 80)
        print("Greedy Boruta特徴量選択（ランキングモデル専用）")
        print("=" * 80)
        print(f"  - 初期特徴量数: {len(X.columns)}個")
        print(f"  - 有意水準: {self.alpha}")
        print(f"  - 最大反復回数: {self.max_iter}")
        print(f"  - 評価指標: NDCG@5（LambdaRank）")
        print("=" * 80)
        ...
        for iteration in range(self.max_iter):
            ...
            # ランキングモデルで重要度を計算
            importances = self._train_ranking_model(X_with_shadow, y, groups)
...
def main():
    parser = argparse.ArgumentParser(description='Phase 7-Ranking: Boruta特徴量選択')
    ...
    print("\n" + "=" * 80)
    print("Phase 7-Ranking: Boruta特徴量選択")
    print("=" * 80)
    print(f"入力: {csv_path}")
    ...
    # 特徴量抽出
    exclude_cols = [
        'race_id', 'kaisai_yen', 'kaisai_tsukihi', 'keibajo_code', 'race_bango',
    ...
    print(f"  - 目的変数: rank_target（範囲: {y.min():.1f} - {y.max():.1f}）")
    
    # 強制保持する特徴量
    force_keep = args.force_keep.split(',') if args.force_keep else []
    
    # Boruta実行
    boruta = GreedyBorutaRanking(
        alpha=args.alpha,
        max_iter=args.max_iter,
        force_keep=force_keep
    )
    
    selected_features = boruta.fit(X, y, groups)
    ...
    # 選択された特徴量をCSVで保存
    output_csv = output_dir / f'{venue_name}_ranking_selected_features.csv'
    ...
    report_path = output_dir / f'{venue_name}_ranking_boruta_report.json'
    ...
    print("\n" + "=" * 80)
    print("Phase 7-Ranking 完了！")
    print("=" * 80)
    print(f"次のステップ: Phase 8-Rankingでハイパーパラメータ最適化")
    print(f"コマンド: python scripts/phase8_auto_tuning/run_optuna_tuning_ranking.py \\")
