...
# -*- coding: utf-8 -*-
"""
Phase 7-8 チューニング済み回帰モデルで予測を実行（予測専用版・全競馬場対応）

Phase8でチューニングされたモデル（*_regression_tuned_model.txt）を使用
Phase7で選択された特徴量（*_regression_selected_features.csv）を使用
"""
...
def predict_optimized_regression(test_csv, models_dir, output_path, selected_features_path=None):
    """
    チューニング済み回帰モデルで予測（全競馬場対応）
    ...
    """
    print(f"\n{'='*80}")
    print(f"Phase 7-8 回帰予測（チューニング済みモデル・全競馬場対応）")
    print(f"{'='*80}")
    ...
    try:
        ...
        model_filename = keibajo_romaji + '_regression_tuned_model.txt'
    ...
    if selected_features_path is None:
        selected_features_path = f"data/features/selected/{keibajo_romaji}_regression_selected_features.csv"
    ...
    # 特徴量の準備
    print(f"\n[4/6] 特徴量の準備")
    exclude_cols = ['target', 'kakutei_chakujun', 'time', 'race_id'] + id_cols
    ...
    print(f"  - 平均予測タイム: {results['Average Predicted Time']:.2f}秒")
    
    # 各レースの最速予測馬を表示
    print(f"\n  レース別最速予測:")
    fastest_predictions = result_df[result_df['time_rank'] == 1]
    for _, row in fastest_predictions.iterrows():
        print(f"    - {row['race_id']}: {row['umaban']}番 (タイム: {row['predicted_time']:.2f}秒)")
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print("使用法: python predict_optimized_regression.py <test_csv> <models_dir> <output_csv> [selected_features_csv]")
        print("\n例:")
        print("  python predict_optimized_regression.py \\")
        ...
        print("    data\\features\\selected\\saga_regression_selected_features.csv")
        print("\n注:")
    ...
    try:
        results = predict_optimized_regression(test_csv, models_dir, output_path, selected_features_path)
        print("\n" + "="*80)
        print("✅ Phase 7-8 回帰予測完了")
        print("="*80)
    except Exception as e:
        print(f"\n❌ エラー発生: {e}")
        import traceback
